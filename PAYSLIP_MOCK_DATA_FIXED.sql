-- =====================================================
-- PAYSLIP MOCK DATA - 5 USERS x 5 MONTHS (FIXED)
-- WorkZen HRMS - Comprehensive Test Data
-- Generated: November 9, 2025
-- Fixed: Handles existing attendance table structure
-- =====================================================

USE workzen_hrms;

-- =====================================================
-- STEP 1: CHECK AND CREATE TABLES WITH CORRECT STRUCTURE
-- =====================================================

-- First, let's check if we need to create a separate payroll schema
-- Or work with the existing structure

-- Option 1: Create separate payroll tables (RECOMMENDED)
-- This keeps payroll data separate from main HRMS data

-- Create employees table for payroll (separate from users/employee_profiles)
CREATE TABLE IF NOT EXISTS payroll_employees (
  id INT PRIMARY KEY AUTO_INCREMENT,
  name VARCHAR(100) NOT NULL,
  emp_id VARCHAR(50) UNIQUE NOT NULL,
  basic_salary DECIMAL(10,2) NOT NULL,
  hra DECIMAL(10,2) DEFAULT 0,
  pf_rate DECIMAL(5,2) DEFAULT 0.12,
  tax_rate DECIMAL(5,2) DEFAULT 0,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Create attendance table for payroll
CREATE TABLE IF NOT EXISTS payroll_attendance (
  id INT PRIMARY KEY AUTO_INCREMENT,
  emp_id INT NOT NULL,
  month VARCHAR(20) NOT NULL,
  year INT NOT NULL,
  present_days INT DEFAULT 0,
  paid_leaves INT DEFAULT 0,
  unpaid_leaves INT DEFAULT 0,
  total_working_days INT DEFAULT 30,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (emp_id) REFERENCES payroll_employees(id) ON DELETE CASCADE,
  UNIQUE KEY unique_attendance (emp_id, month, year)
);

-- Create payruns table
CREATE TABLE IF NOT EXISTS payroll_payruns (
  id INT PRIMARY KEY AUTO_INCREMENT,
  month VARCHAR(20) NOT NULL,
  year INT NOT NULL,
  total_employees INT DEFAULT 0,
  total_cost DECIMAL(12,2) DEFAULT 0,
  status ENUM('Pending', 'Done', 'Validated') DEFAULT 'Pending',
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  UNIQUE KEY unique_payrun (month, year)
);

-- Create payslips table
CREATE TABLE IF NOT EXISTS payroll_payslips (
  id INT PRIMARY KEY AUTO_INCREMENT,
  emp_id INT NOT NULL,
  payrun_id INT NULL,
  month VARCHAR(20) NOT NULL,
  year INT NOT NULL,
  basic_salary DECIMAL(10,2) DEFAULT 0,
  hra DECIMAL(10,2) DEFAULT 0,
  earned_salary DECIMAL(10,2) DEFAULT 0,
  gross_salary DECIMAL(10,2) DEFAULT 0,
  pf_deduction DECIMAL(10,2) DEFAULT 0,
  tax_deduction DECIMAL(10,2) DEFAULT 0,
  unpaid_deduction DECIMAL(10,2) DEFAULT 0,
  total_deductions DECIMAL(10,2) DEFAULT 0,
  net_salary DECIMAL(10,2) DEFAULT 0,
  present_days INT DEFAULT 0,
  paid_leaves INT DEFAULT 0,
  unpaid_leaves INT DEFAULT 0,
  status ENUM('Draft','Done') DEFAULT 'Draft',
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (emp_id) REFERENCES payroll_employees(id) ON DELETE CASCADE,
  FOREIGN KEY (payrun_id) REFERENCES payroll_payruns(id) ON DELETE CASCADE,
  UNIQUE KEY unique_emp_month_year (emp_id, month, year)
);

-- =====================================================
-- STEP 2: INSERT 5 EMPLOYEES
-- =====================================================

INSERT INTO payroll_employees (name, emp_id, basic_salary, hra, pf_rate, tax_rate) VALUES
('Rajesh Kumar', 'EMP001', 25000.00, 5000.00, 0.12, 0),
('Priya Sharma', 'EMP002', 30000.00, 6000.00, 0.12, 0),
('Amit Patel', 'EMP003', 28000.00, 5600.00, 0.12, 0),
('Sneha Reddy', 'EMP004', 32000.00, 6400.00, 0.12, 0),
('Vikram Singh', 'EMP005', 27000.00, 5400.00, 0.12, 0)
ON DUPLICATE KEY UPDATE 
  name = VALUES(name),
  basic_salary = VALUES(basic_salary),
  hra = VALUES(hra),
  pf_rate = VALUES(pf_rate),
  tax_rate = VALUES(tax_rate);

-- =====================================================
-- STEP 3: INSERT ATTENDANCE DATA FOR 5 MONTHS
-- =====================================================

-- JUNE 2025
INSERT INTO payroll_attendance (emp_id, month, year, present_days, paid_leaves, unpaid_leaves, total_working_days) VALUES
(1, 'June', 2025, 22, 2, 0, 30),
(2, 'June', 2025, 24, 1, 0, 30),
(3, 'June', 2025, 20, 2, 2, 30),
(4, 'June', 2025, 23, 1, 0, 30),
(5, 'June', 2025, 21, 3, 0, 30)
ON DUPLICATE KEY UPDATE 
  present_days = VALUES(present_days),
  paid_leaves = VALUES(paid_leaves),
  unpaid_leaves = VALUES(unpaid_leaves);

-- JULY 2025
INSERT INTO payroll_attendance (emp_id, month, year, present_days, paid_leaves, unpaid_leaves, total_working_days) VALUES
(1, 'July', 2025, 20, 3, 1, 30),
(2, 'July', 2025, 23, 2, 0, 30),
(3, 'July', 2025, 22, 1, 1, 30),
(4, 'July', 2025, 24, 0, 0, 30),
(5, 'July', 2025, 19, 4, 1, 30)
ON DUPLICATE KEY UPDATE 
  present_days = VALUES(present_days),
  paid_leaves = VALUES(paid_leaves),
  unpaid_leaves = VALUES(unpaid_leaves);

-- AUGUST 2025
INSERT INTO payroll_attendance (emp_id, month, year, present_days, paid_leaves, unpaid_leaves, total_working_days) VALUES
(1, 'August', 2025, 23, 1, 0, 30),
(2, 'August', 2025, 22, 2, 0, 30),
(3, 'August', 2025, 21, 2, 1, 30),
(4, 'August', 2025, 23, 2, 0, 30),
(5, 'August', 2025, 22, 2, 0, 30)
ON DUPLICATE KEY UPDATE 
  present_days = VALUES(present_days),
  paid_leaves = VALUES(paid_leaves),
  unpaid_leaves = VALUES(unpaid_leaves);

-- SEPTEMBER 2025
INSERT INTO payroll_attendance (emp_id, month, year, present_days, paid_leaves, unpaid_leaves, total_working_days) VALUES
(1, 'September', 2025, 20, 2, 2, 30),
(2, 'September', 2025, 22, 2, 0, 30),
(3, 'September', 2025, 21, 1, 2, 30),
(4, 'September', 2025, 23, 1, 0, 30),
(5, 'September', 2025, 20, 2, 2, 30)
ON DUPLICATE KEY UPDATE 
  present_days = VALUES(present_days),
  paid_leaves = VALUES(paid_leaves),
  unpaid_leaves = VALUES(unpaid_leaves);

-- OCTOBER 2025
INSERT INTO payroll_attendance (emp_id, month, year, present_days, paid_leaves, unpaid_leaves, total_working_days) VALUES
(1, 'October', 2025, 22, 2, 0, 30),
(2, 'October', 2025, 24, 0, 0, 30),
(3, 'October', 2025, 20, 2, 2, 30),
(4, 'October', 2025, 23, 1, 0, 30),
(5, 'October', 2025, 21, 3, 0, 30)
ON DUPLICATE KEY UPDATE 
  present_days = VALUES(present_days),
  paid_leaves = VALUES(paid_leaves),
  unpaid_leaves = VALUES(unpaid_leaves);

-- =====================================================
-- STEP 4: VERIFICATION QUERIES
-- =====================================================

-- View all employees
SELECT 
    '========== EMPLOYEES ==========' as '';

SELECT 
    id,
    name,
    emp_id,
    CONCAT('₹', FORMAT(basic_salary, 2)) as basic_salary,
    CONCAT('₹', FORMAT(hra, 2)) as hra,
    CONCAT((pf_rate * 100), '%') as pf_rate
FROM payroll_employees
ORDER BY id;

-- View attendance summary
SELECT 
    '========== ATTENDANCE SUMMARY ==========' as '';

SELECT 
    e.emp_id,
    e.name,
    a.month,
    a.year,
    a.present_days,
    a.paid_leaves,
    a.unpaid_leaves,
    (a.present_days + a.paid_leaves) as earned_days
FROM payroll_employees e
JOIN payroll_attendance a ON e.id = a.emp_id
ORDER BY a.year, 
    FIELD(a.month, 'June', 'July', 'August', 'September', 'October'),
    e.id;

-- Sample salary calculations for October 2025
SELECT 
    '========== OCTOBER 2025 SALARY PREVIEW ==========' as '';

SELECT 
    e.emp_id,
    e.name,
    a.present_days,
    a.paid_leaves,
    a.unpaid_leaves,
    CONCAT('₹', FORMAT(e.basic_salary, 2)) as basic_salary,
    CONCAT('₹', FORMAT(
        ROUND((e.basic_salary / 30) * (a.present_days + a.paid_leaves) * 1.20 - 
              ((e.basic_salary / 30) * (a.present_days + a.paid_leaves) * 0.12 + 200 + 
               (e.basic_salary / 30) * a.unpaid_leaves), 2), 2
    )) as estimated_net_salary
FROM payroll_employees e
JOIN payroll_attendance a ON e.id = a.emp_id
WHERE a.month = 'October' AND a.year = 2025
ORDER BY e.id;

-- Success message
SELECT 
    '========== STATUS ==========' as '';

SELECT 
    '✅ Data inserted successfully!' as Status,
    COUNT(DISTINCT e.id) as Total_Employees,
    COUNT(a.id) as Total_Attendance_Records,
    COUNT(DISTINCT CONCAT(a.month, ' ', a.year)) as Months_Covered
FROM payroll_employees e
LEFT JOIN payroll_attendance a ON e.id = a.emp_id;

-- =====================================================
-- IMPORTANT: UPDATE CONTROLLER REFERENCES
-- =====================================================
-- 
-- You need to update your backend controllers to use these new table names:
-- - payroll_employees (instead of employees)
-- - payroll_attendance (instead of attendance)
-- - payroll_payslips (instead of payslips)
-- - payroll_payruns (instead of payruns)
--
-- This prevents conflicts with the main HRMS tables.
--
-- =====================================================
