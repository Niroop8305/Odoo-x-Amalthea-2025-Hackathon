...
const EmployeeAttendance = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // Calculate summary
        const presentDays = attendanceData.filter(r => r.check_in_time || r.checkIn).length;
        
        // Try to fetch leaves count
        let leavesCount = 0;
        try {
          const leavesResp = await api.get(`/leave/my-leaves?month=${month}&year=${year}`);
          leavesCount = leavesResp.data?.data?.filter(l => l.status === 'Approved').length || 0;
        } catch (err) {
  ...
  const monthDays = () => {
    ...
    for (let d = new Date(firstDay); d <= lastDay; d.setDate(d.getDate() + 1)) {
      ...
      const record = records.find(r => (r.date || r.day || '').startsWith(iso));
      days.push({
        date: new Date(d),
        record: record || null
      });
          ...
          <table className="ea-table">
            ...
            <tbody>
              {records.length === 0 ? (
                ...
                records.map((record) => {
                  const recordDate = new Date(record.date || record.day || record.attendance_date);
                  const dateStr = recordDate.toLocaleDateString('en-GB'); // DD/MM/YYYY format
                  const checkIn = record.check_in_time || record.checkIn || '--:--';
                  const checkOut = record.check_out_time || record.checkOut || '--:--';
