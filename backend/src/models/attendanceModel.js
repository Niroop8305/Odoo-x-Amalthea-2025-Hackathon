...
class AttendanceModel {
  ...
  static async getAttendance(empId, month, year) {
    const [rows] = await pool.query(
      "SELECT * FROM payroll_attendance WHERE emp_id = ? AND month = ? AND year = ?",
      [empId, month, year]
    );
    return rows[0];
  }
  ...
  static async getAllAttendance() {
    const [rows] = await pool.query(
      "SELECT * FROM payroll_attendance ORDER BY year DESC, month DESC"
    );
    return rows;
  }
  ...
  static async upsertAttendance(attendanceData) {
    ...
    const [result] = await pool.query(
      `INSERT INTO payroll_attendance (emp_id, month, year, present_days, paid_leaves, unpaid_leaves, total_working_days) 
       VALUES (?, ?, ?, ?, ?, ?, ?)
  ...
  static async getAttendanceByPeriod(month, year) {
    const [rows] = await pool.query(
      `SELECT a.*, e.name, e.emp_id as employee_id, e.basic_salary, e.hra, e.pf_rate, e.tax_rate
       ...
       JOIN payroll_employees e ON a.emp_id = e.id
  ...
  static async deleteAttendance(id) {
    const [result] = await pool.query(
      "DELETE FROM payroll_attendance WHERE id = ?",
      [id]
    );
